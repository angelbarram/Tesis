...
train_dataset = PulsarDataset(image_folder='../datos/data_pool_fine_tuning/train', transform=transform)
val_dataset = PulsarDataset(image_folder='../datos/data_pool_fine_tuning/validation', transform=transform)
test_dataset = PulsarDataset(image_folder='../datos/data_pool_fine_tuning/test', transform=transform)

# Definición dela CNN
class SimpleCNN(nn.Module):
    def __init__(self, dropout_rate_fc1=0.2):
        super(SimpleCNN, self).__init__()
        self.conv1 = nn.Conv2d(1, 32, kernel_size=3, stride=1, padding=1)
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.conv2 = nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1)
        self.fc1 = nn.Linear(64 * 32 * 32, 128)
        self.dropout_fc1 = nn.Dropout(p=dropout_rate_fc1)
...
best_dropout_fc1 = 0.2

# Cargar el modelo preentrenado
model = SimpleCNN(dropout_rate_fc1=best_dropout_fc1).to(device)
model.load_state_dict(torch.load('model_for_fine_tuning.pth'))

# Congelar las capas convolucionales para el ajuste fino
for param in model.conv1.parameters():
    param.requires_grad = False
for param in model.conv2.parameters():
    param.requires_grad = True

# Modificar la capa de salida (fc2) para el ajuste fino
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=100, patience=5):
    ...
    if best_model_state is not None:
        ...
        torch.save(model.state_dict(), 'best_model_full_data_final.pth')
...
os.makedirs('training_validation_images_full', exist_ok=True)
...
plt.savefig('training_validation_images_full/loss_curves.png')
plt.close()
...
plt.savefig('training_validation_images_full/accuracy_curves.png')
plt.close()
...
logging.basicConfig(filename='test_metrics_full_10.log', level=logging.INFO, 
...
plt.savefig('training_validation_images_full/normalized_confusion_matrix.png')
plt.close()
